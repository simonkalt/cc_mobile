...
def test_stripe_connectivity():
    ...
    if STRIPE_AVAILABLE:
        ...
        result["stripe_library"][
            "error"
        ] = "Stripe library not installed. Install with: pip install stripe>=7.0.0"
    ...
    result["environment_variables"] = {
        "STRIPE_TEST_API_KEY": {
            ...
            "value_preview": (
                settings.STRIPE_TEST_API_KEY[:10] + "..."
                if settings.STRIPE_TEST_API_KEY and len(settings.STRIPE_TEST_API_KEY) > 10
                else None
            ),
            "key_type": (
                "test"
                if settings.STRIPE_TEST_API_KEY
                and settings.STRIPE_TEST_API_KEY.startswith("sk_test_")
                else (
                    "live"
                    if settings.STRIPE_TEST_API_KEY
                    and settings.STRIPE_TEST_API_KEY.startswith("sk_live_")
                    else "unknown"
                )
            ),
        },
        "STRIPE_API_KEY": {
            ...
            "value_preview": (
                settings.STRIPE_API_KEY[:10] + "..."
                if settings.STRIPE_API_KEY and len(settings.STRIPE_API_KEY) > 10
                else None
            ),
            "key_type": (
                "production"
        ...
        "STRIPE_WEBHOOK_SECRET": {
            ...
            "value_preview": (
                settings.STRIPE_WEBHOOK_SECRET[:10] + "..."
                if settings.STRIPE_WEBHOOK_SECRET and len(settings.STRIPE_WEBHOOK_SECRET) > 10
                else None
            ),
        },
    ...
    result["environment_variables"]["active_api_key"] = {
        "source": (
            ...
            else ("STRIPE_TEST_API_KEY" if settings.STRIPE_TEST_API_KEY else None)
        ),
        "key_type": (
            "production"
            if settings.STRIPE_API_KEY
            else ("test" if settings.STRIPE_TEST_API_KEY else None)
        ),
    ...
    if not STRIPE_AVAILABLE:
        ...
        result["summary"][
            "message"
        ] = "❌ Stripe library is not available. Install with: pip install stripe>=7.0.0"
        return result
    ...
    if not active_api_key:
        ...
        result["summary"][
            "message"
        ] = "❌ No Stripe API key found. Set STRIPE_API_KEY or STRIPE_TEST_API_KEY in environment variables."
        return result
    ...
    try:
        ...
        if account_test.get("status") == "success":
            ...
            result["summary"][
                "message"
            ] = f"❌ Stripe error: {account_test.get('error', 'Unknown error')}"
